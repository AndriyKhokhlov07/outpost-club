...
<div id="customer">
	...
	<table>		
		<tr>
			<td><h2><i>Note:</i></h2><i>{$order->note|escape|nl2br}</i></td>
		</tr>
	</table>
	{/if}
	*}
...
<div id="map">
	<iframe width="550" height="370" frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="http://maps.google.com/maps?ie=UTF8&iwloc=near&hl=en&t=m&z=16&mrt=loc&geocode=&q={$order->address|escape|urlencode}&output=embed"></iframe>
</div>
		...
		<tr>
			<td>
				<span class=view_purchase>
					{$purchase->product_name} {$purchase->variant_name} {if $purchase->sku} (SKU {$purchase->sku}){/if}			
				</span>
			</td>
...
<div id="total">
	<table>
		...
		<tr>
			<th>Coupon{if $order->coupon_code} ({$order->coupon_code}){/if}</th>
		...
		{if $payment_method}
		<tr>
			<td colspan="2">Payment method: {$payment_method->name}</td>
		</tr>
		<tr>
			<th>Total to pay</th>
